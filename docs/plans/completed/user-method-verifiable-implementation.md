...
**Solution**: Add `_isVerifiable` and `_verifiableTimes` fields, update `Verifiable()` methods to set state, add `CheckVerification()` method, and include stub overrides in `Stub.Verify()`.
...
4. Include stub overrides in `Stub.Verify()` aggregation loop

Stub overrides should NOT be included in `VerifyAll()` because they are always "configured" (user provides implementation), so including them would require every stub override to be called.
...
2. **Guard condition at line 2114** (CRITICAL - discovered during developer review)
   ...
   - `RenderVerifyMethods` is never called for stubs with only stub overrides, properties, indexers, or events
   - Must expand to include all verifiable member types

3. **`RenderVerifyMethods`** (lines 2126-2232)
   - Add stub override interceptors to `Verify()` aggregation (after line 2188)
   - Note: Do NOT add to `VerifyAll()` - stub overrides are always "configured"
...
// Must check all member types: methods, stub overrides, properties, indexers, events
...
- Stubs with only stub overrides, properties, indexers, or events would not have `Verify()` or `VerifyAll()` methods
- Evidence: `StrictModeUserMethodStub.g.cs` has stub overrides and a property but NO `Verify()`/`VerifyAll()` methods

#### 5. Add Stub Overrides to Verify() Aggregation (after line 2188)
...
2. Add `CheckVerification()` method to stub override interceptor class
...
5. Modify `RenderVerifyMethods` to include stub overrides in `Verify()` aggregation
6. Add tests for stub override Verifiable behavior
...
- [ ] Stub override interceptors have `_isVerifiable` and `_verifiableTimes` fields
...
- [ ] `Stub.Verify()` includes stub override interceptors
- [ ] `Stub.VerifyAll()` does NOT include stub override interceptors
- [ ] Tests verify all scenarios
- [ ] **New test**: Stub with ONLY stub overrides has working `Verify()`/`VerifyAll()` methods
...
3. **Generic stub overrides**: User-defined methods cannot be generic (filtered out by `!m.IsGenericMethod`), so no special handling needed.
...
| **Inline Interface** | NO | No partial class = no stub overrides possible |
| **Inline Class** | NO | No partial class = no stub overrides possible |
| **Inline Delegate** | NO | No partial class = no stub overrides possible |
...
- The `Verifiable()` API already exists on stub override interceptors
...
| Feature | Regular Methods | Stub Overrides (Current) | Stub Overrides (After Fix) |
|---------|----------------|------------------------|-------------------------|
...
3. **`/home/keithvoels/neatoodotnet/KnockOff/src/Tests/KnockOffTests/Generated/KnockOff.Generator/KnockOff.KnockOffGenerator/SampleKnockOff.g.cs`**
   - Lines 1370-1404: Generated `GetValue2Interceptor` (stub override interceptor)
...
4. **`/home/keithvoels/neatoodotnet/KnockOff/src/Tests/KnockOffTests/TestInterfaces.cs`**
   - Line 17: Example stub override `protected int GetValue(int input)`
   - Lines 377-386: Additional stub override examples in `StrictModeUserMethodStub`
...
1. **Multiple stub overrides on same stub**: Each should be independently verifiable and independently fail in `Stub.Verify()`

2. **Stub overrides with different parameter counts**: The interceptor class name varies (e.g., `GetValue2Interceptor`), but the `method.MethodName` gives the actual name for error messages

3. **Reset behavior**: `Reset()` should NOT clear `_isVerifiable` - verified this matches regular method behavior. The current stub override `Reset()` only clears `CallCount` and `_lastArg`, which is correct.

4. **Interaction with individual Verify()**: Stub overrides already have `Verify()` and `Verify(Times)` methods that throw on failure. The new `CheckVerification()` is for aggregate verification via `Stub.Verify()` only.

5. **Generic stub overrides**: Not applicable - user-defined methods for generic interface methods use `GenericMethodHandler`, not `RenderUserMethodInterceptorClass`. Verified by filter: `!m.IsGenericMethod && m.UserMethodCall != null`
...
2. Evidence confirmed: `StrictModeUserMethodStub.g.cs` has NO `Verify()`/`VerifyAll()` methods despite having stub overrides and a property
...
3. **[Clarification]: CallCount vs TotalCallCount** - CONFIRMED
   - Correct: stub overrides have no sequences, so `CallCount` equals total
...
- [ ] Line ~2188: Add stub overrides to `Verify()` aggregation loop in `RenderVerifyMethods`

**File: `/home/keithvoels/neatoodotnet/KnockOff/src/Tests/KnockOffTests/UserMethodVerificationTests.cs`** (NEW)

- [ ] Test: Stub override marked Verifiable, called - passes
- [ ] Test: Stub override marked Verifiable, NOT called - throws
- [ ] Test: Stub override marked Verifiable(Times.Exactly(2)) - verifies count
- [ ] Test: Multiple stub overrides both fail - exception contains both
- [ ] Test: Stub override NOT marked Verifiable - not checked
- [ ] Test: VerifyAll does NOT include stub overrides
- [ ] Test: Stub with ONLY stub overrides has working Verify() (guard condition regression test)

### Explicitly Out of Scope

- `VerifyAll()` changes for stub overrides (intentionally excluded - stub overrides are always "configured")
- Regular method interceptors (already working correctly)
- Inline patterns (stub overrides only exist in Standalone pattern)
...
1. **After Phase 1** (RenderUserMethodInterceptorClass changes):
   - Build succeeds
   - Generated stub override interceptors have `_isVerifiable`, `_verifiableTimes` fields
   - Generated stub override interceptors have `CheckVerification()` method
...
3. **After Phase 3** (Verify aggregation):
   - Build succeeds
   - Stub override interceptors included in `Verify()` loop in generated code
...
- Architectural contradiction discovered (e.g., stub overrides need to be in VerifyAll after all)
...
- [x] Add stub overrides to `Verify()` aggregation loop
- [x] **Verification**: Build passes

**Phase 4:** Add Tests
- [x] Stub override marked Verifiable, called → passes
- [x] Stub override marked Verifiable, NOT called → throws
- [x] Stub override marked Verifiable(Times.Exactly(2)) → verifies count
- [x] Multiple stub overrides both fail → exception contains both
- [x] Stub override NOT marked Verifiable → not checked
- [x] VerifyAll does NOT include stub overrides
- [x] **Stub with ONLY stub overrides** → `Verify()` works correctly (guard condition test)
...
Stub override interceptor with verifiable state:
```csharp
...
Stub.Verify() now includes stub override interceptors:
```csharp
public void Verify()
{
...
1. **Stub override marked Verifiable, called** → `Stub.Verify()` passes
2. **Stub override marked Verifiable, NOT called** → `Stub.Verify()` throws
3. **Stub override marked Verifiable(Times.Exactly(2))** → Verifies exact count
4. **Multiple stub overrides, both fail** → Exception contains both failures
5. **Stub override NOT marked Verifiable** → NOT checked by `Stub.Verify()`
6. **Stub override Reset** → Preserves verifiable marking, resets call count
7. **VerifyAll** → Does NOT include stub overrides (intentional)
8. **Stub with ONLY stub overrides** → `Stub.Verify()` method exists and works correctly (guard condition regression test)
