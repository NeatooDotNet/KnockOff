# v0.9.0 - Flat API & Class Stub Composition

**Release Date:** TBD
**NuGet:** [KnockOff 0.9.0](https://nuget.org/packages/KnockOff/0.9.0)
...
**Before (v0.8.0):** Interceptors were accessed via interface-named properties:

```csharp
var stub = new UserServiceKnockOff();
stub.IUserService.GetUser.WasCalled;
stub.IUserService.GetUser.OnCall = ((id) => new User { Id = id };
stub.IUserService.Name.SetCount;
```

**After (v0.9.0):** Interceptors are accessed directly on the stub:
...
**Before (v0.8.0):** Class stubs inherited from the target class and accessed interceptors via `.Interceptor`:

```csharp
var stub = new Stubs.UserService();
stub.Interceptor.GetUser.OnCall = ((id) => new User { Id = id };
UserService service = stub;  // Direct assignment
```

**After (v0.9.0):** Class stubs use composition with direct interceptor access and `.Object` property:

```csharp
var stub = new Stubs.UserService();
stub.GetUser.OnCall = ((id) => new User { Id = id };  // Direct access!
...
**Full Changelog**: https://github.com/NeatooDotNet/KnockOff/compare/v0.8.0...v0.9.0
