# v0.21.0 - Interceptor API Redesign
...
// After
var tracking = stub.DoWork.OnCall(() => result);
...
stub.Format.OnCall((input, uppercase) => uppercase ? input.ToUpper() : input);
stub.Format.OnCall((input, maxLength) => input[..Math.Min(input.Length, maxLength)]);
...
```csharp
stub.GetNext.OnCall(() => 1, Times.Once)
...
| `stub.Method1.OnCall = ...` | `stub.Method.OnCall((param) => ...)` |
