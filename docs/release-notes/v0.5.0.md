# v0.5.0 - Interface Spy Architecture

**Release Date:** 2026-01-03
**NuGet:** [KnockOff 0.5.0](https://nuget.org/packages/KnockOff/0.5.0)
...
**After:**
```csharp
knockOff.IRepository.GetById.OnCall = ((id) => new User { Id = id };
```
...
knockOff.IService.Process1.OnCall = ((data) => { };           // Process(string)
knockOff.IService.Process2.OnCall = ((data, priority) => { }; // Process(string, int)
...
**After:**
```csharp
knockOff.IRepository.Create.OnCall = ((name, value) =>
...
knockOff.ICalculator.Calculate1.OnCall = ((value) => value * 2;
knockOff.ICalculator.Calculate2.OnCall = ((a, b) => a + b;
...
2. Replace `.OnCall((` with `.OnCall = (` (remove explicit types for most cases):
   - `.OnCall((ServiceKnockOff ko, int id) =>` → `.OnCall = ((id) =>`
...
knockOff.IUserService.GetUser.OnCall = ((id) =>
    new User { Id = id };
knockOff.IUserService.Name.OnGet = () => "Test";
...
knockOff.IParser.TryParse.OnCall =
    (IParser_TryParseHandler.TryParseDelegate)(((string input, out int result) =>
    {
...
**Full Changelog**: https://github.com/NeatooDotNet/KnockOff/compare/v0.4.1...v0.5.0
