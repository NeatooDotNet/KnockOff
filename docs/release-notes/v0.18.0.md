# v0.18.0 - Open Generic Inline Interface Stubs
...
store.Set.OnCall = ((key, value) => Console.WriteLine($"{key}={value}");
```
...
- Open generic class stubs (`[KnockOff(typeof(SomeClass<>))]`) - added in v0.20
- Open generic delegate stubs (`[KnockOff(typeof(Factory<>))]`) - added in v0.20
