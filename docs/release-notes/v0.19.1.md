...
stub.Fetch1.OnCall = ((id) => Task.FromResult<IArea?>(result); // Async
stub.Fetch2.OnCall = ((entity) => result;                       // Sync
```
...
stub.Execute1.OnCall = ((target, ct) => result; // Takes ISpecificTarget
stub.Execute2.OnCall = ((target, ct) => result; // Takes ITarget
```
...
2. Compatibility is checked - methods can share an interceptor if:
   - Parameter types are compatible (same name → same type)
...
No migration required. This is a drop-in replacement for v0.19.0.
