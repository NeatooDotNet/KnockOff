...
KnockOff provides `Return(value)` for methods and `OnGet(value)` for properties, allowing you to configure a static return value directly.
...
**Solution:** Configure the return value using `Return` for methods or `OnGet` for properties.
