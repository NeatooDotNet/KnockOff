...
- **Generic delegates**: Both closed (`[KnockOff<Factory<string>>]`) and open (`[KnockOff(typeof(Factory<>))]`) generic delegates supported. *(Open generic support added in v0.20)*
...
- **Open generic delegates**: Now supported as of v0.20 via `[KnockOff(typeof(Factory<>))]` syntax.
