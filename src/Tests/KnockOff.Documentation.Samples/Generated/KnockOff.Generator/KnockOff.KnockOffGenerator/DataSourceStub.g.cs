...
partial class DataSourceStub : global::KnockOff.Documentation.Samples.SourceDelegation.IDataSource, global::KnockOff.IKnockOffStub
{
	/// <summary>Tracks and configures behavior for Read.</summary>
	public sealed class ReadInterceptor
	{
		/// <summary>Source object to delegate to when no OnCall is configured.</summary>
		internal global::KnockOff.Documentation.Samples.SourceDelegation.IDataSource? _source;

		/// <summary>Delegate for Read.</summary>
		public delegate string? ReadDelegate(string filename);
		...
		public global::KnockOff.IMethodTracking<string> OnCall(ReadDelegate callback)
		{
			_sequence = null;
			_sequenceIndex = 0;
			_isVerifiable = false;
		...
		internal string? Invoke(bool strict, string filename)
		{
			if (_sequence != null && _sequenceIndex < _sequence.Count)
			{
				var (callback, tracking) = _sequence[_sequenceIndex];
				tracking.RecordCall(filename);
				_sequenceIndex++;
			...
			{
				_onCallValueTracking.RecordCall(filename);
				return _onCallValue;
			}

			if (_onCall != null && _onCallTracking != null)
			{
				_onCallTracking.RecordCall(filename);
				return _onCall(filename);
			}

			_unconfiguredCallCount++;
			_unconfiguredLastArg = filename;
			if (_sequence != null && _sequenceIndex >= _sequence.Count)
			{
			...
			if (_source is { } src) return src.Read(filename);
		...
		{
			private readonly ReadInterceptor _interceptor;

			public MethodTrackingImpl(ReadInterceptor interceptor) => _interceptor = interceptor;
			...
			public void RecordCall(string filename) { CallCount++; _lastArg = filename; }
		...
		private sealed class MethodSequenceImpl : global::KnockOff.IMethodSequence<ReadDelegate>
		{
			...
			public MethodSequenceImpl(ReadInterceptor interceptor) => _interceptor = interceptor;
	...
	/// <summary>Tracks and configures behavior for Write.</summary>
	public sealed class WriteInterceptor
	{
		/// <summary>Source object to delegate to when no OnCall is configured.</summary>
		internal global::KnockOff.Documentation.Samples.SourceDelegation.IDataSource? _source;
		...
		private int _unconfiguredCallCount;
		private (string? filename, string? content)? _unconfiguredLastArgs;
		...
		{
			if (!times.Validate(TotalCallCount))
				throw new global::KnockOff.VerificationException(new global::KnockOff.VerificationFailure("Write", times, TotalCallCount));
		}

		/// <summary>Configures callback that repeats indefinitely. Returns tracking interface for LastArg access.</summary>
		public global::KnockOff.IMethodTrackingArgs<(string? filename, string? content)> OnCall(global::System.Action<string, string> callback)
		{
			_sequence = null;
			_sequenceIndex = 0;
			_isVerifiable = false;
		...
		/// <summary>Invokes the configured callback. Called by explicit interface implementation.</summary>
		internal void Invoke(bool strict, string filename, string content)
		{
			if (_sequence != null && _sequenceIndex < _sequence.Count)
			{
				var (callback, tracking) = _sequence[_sequenceIndex];
				tracking.RecordCall((filename, content));
				_sequenceIndex++;
			...
			if (_onCall != null && _onCallTracking != null)
			{
				_onCallTracking.RecordCall((filename, content));
				_onCall(filename, content);
				return;
			}

			_unconfiguredCallCount++;
			_unconfiguredLastArgs = ((filename, content));
			if (_sequence != null && _sequenceIndex >= _sequence.Count)
			{
				if (strict) throw global::KnockOff.StubException.SequenceExhausted("Write");
				return;
			}

			#pragma warning disable CS8601, SYSLIB0050
			if (_source is { } src) { src.Write(filename, content); return; }
			#pragma warning restore CS8601, SYSLIB0050
			if (strict) throw global::KnockOff.StubException.NotConfigured("", "Write");
			return;
		}
		...
		internal global::KnockOff.VerificationFailure? CheckVerification()
		{
			if (!_isVerifiable) return null;
			var times = _verifiableTimes ?? global::KnockOff.Times.AtLeastOnce;
			return times.Validate(TotalCallCount) ? null : new global::KnockOff.VerificationFailure("Write", times, TotalCallCount);
		}
		...
		{
			if (!IsConfigured) return null;
			return global::KnockOff.Times.AtLeastOnce.Validate(TotalCallCount) ? null : new global::KnockOff.VerificationFailure("Write", global::KnockOff.Times.AtLeastOnce, TotalCallCount);
		}

		/// <summary>Tracks invocations for this callback registration.</summary>
		private sealed class MethodTrackingImpl : global::KnockOff.IMethodTrackingArgs<(string? filename, string? content)>
		{
			private readonly WriteInterceptor _interceptor;

			public MethodTrackingImpl(WriteInterceptor interceptor) => _interceptor = interceptor;

			private (string? filename, string? content) _lastArgs;

			internal int CallCount { get; private set; }

			/// <summary>Last arguments passed to this callback. Default if never called.</summary>
			public (string? filename, string? content) LastArgs => _lastArgs;

			/// <summary>Records a call to this callback.</summary>
			public void RecordCall((string? filename, string? content) args) { CallCount++; _lastArgs = args; }
			...
			/// <summary>Marks for verification by Stub.Verify(). Returns this for fluent chaining.</summary>
			public global::KnockOff.IMethodTrackingArgs<(string? filename, string? content)> Verifiable()
			{
				_interceptor._isVerifiable = true;
				_interceptor._verifiableTimes = null;
				return this;
			...
			/// <summary>Marks for verification by Stub.Verify() with Times constraint. Returns this for fluent chaining.</summary>
			public global::KnockOff.IMethodTrackingArgs<(string? filename, string? content)> Verifiable(global::KnockOff.Times times)
			{
				_interceptor._isVerifiable = true;
				_interceptor._verifiableTimes = times;
				return this;
		...
		private sealed class MethodSequenceImpl : global::KnockOff.IMethodSequence<global::System.Action<string, string>>
		{
			...
			public MethodSequenceImpl(WriteInterceptor interceptor) => _interceptor = interceptor;
	...
	public ReadInterceptor Read { get; } = new();

	/// <summary>Interceptor for Write.</summary>
	public WriteInterceptor Write { get; } = new();

	/// <summary>When true, throws StubException for unconfigured member access.</summary>
	public bool Strict { get; set; } = false;

	/// <summary>The global::KnockOff.Documentation.Samples.SourceDelegation.IDataSource instance. Use for passing to code expecting the interface.</summary>
	public global::KnockOff.Documentation.Samples.SourceDelegation.IDataSource Object => this;
	...
	public void Verify()
	{
		...
		if (Write.CheckVerification() is { } writeFailure) failures.Add(writeFailure);
	...
	public void VerifyAll()
	{
		...
		if (Write.CheckVerificationAll() is { } writeFailure) failures.Add(writeFailure);
	...
	/// <summary>Delegates unconfigured member access to the provided source object (global::KnockOff.Documentation.Samples.SourceDelegation.IDataSource).</summary>
	/// <param name="source">The source to delegate to, or null to clear.</param>
	public void Source(global::KnockOff.Documentation.Samples.SourceDelegation.IDataSource? source)
	{
		Read._source = source;
		Write._source = source;
	}

	string? global::KnockOff.Documentation.Samples.SourceDelegation.IDataSource.Read(string filename)
	{
		return Read.Invoke(Strict, filename);
	}

	void global::KnockOff.Documentation.Samples.SourceDelegation.IDataSource.Write(string filename, string content)
	{
		Write.Invoke(Strict, filename, content);
	}
