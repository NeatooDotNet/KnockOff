...
partial class SkillNotifierStub : global::KnockOff.Documentation.Samples.Skill.ISkillNotifier, global::KnockOff.IKnockOffStub
{
	/// <summary>Tracks and configures behavior for Notify.</summary>
	public sealed class NotifyInterceptor
	{
		/// <summary>Source object to delegate to when no OnCall is configured.</summary>
		internal global::KnockOff.Documentation.Samples.Skill.ISkillNotifier? _source;
		...
		private (int? userId, string? message)? _unconfiguredLastArgs;
		...
		public global::KnockOff.IMethodTrackingArgs<(int? userId, string? message)> OnCall(global::System.Action<int, string> callback)
		{
			_sequence = null;
			_sequenceIndex = 0;
			_isVerifiable = false;
		...
		internal void Invoke(bool strict, int userId, string message)
		{
			if (_sequence != null && _sequenceIndex < _sequence.Count)
			{
				var (callback, tracking) = _sequence[_sequenceIndex];
				tracking.RecordCall((userId, message));
				_sequenceIndex++;
			...
			{
				_onCallTracking.RecordCall((userId, message));
				_onCall(userId, message);
				return;
			}

			_unconfiguredCallCount++;
			_unconfiguredLastArgs = ((userId, message));
			if (_sequence != null && _sequenceIndex >= _sequence.Count)
			{
			...
			if (_source is { } src) { src.Notify(userId, message); return; }
		...
		private sealed class MethodTrackingImpl : global::KnockOff.IMethodTrackingArgs<(int? userId, string? message)>
		{
			private readonly NotifyInterceptor _interceptor;

			public MethodTrackingImpl(NotifyInterceptor interceptor) => _interceptor = interceptor;

			private (int? userId, string? message) _lastArgs;

			internal int CallCount { get; private set; }

			/// <summary>Last arguments passed to this callback. Default if never called.</summary>
			public (int? userId, string? message) LastArgs => _lastArgs;

			/// <summary>Records a call to this callback.</summary>
			public void RecordCall((int? userId, string? message) args) { CallCount++; _lastArgs = args; }
			...
			public global::KnockOff.IMethodTrackingArgs<(int? userId, string? message)> Verifiable()
			{
			...
			public global::KnockOff.IMethodTrackingArgs<(int? userId, string? message)> Verifiable(global::KnockOff.Times times)
			{
				_interceptor._isVerifiable = true;
	...
	/// <summary>The global::KnockOff.Documentation.Samples.Skill.ISkillNotifier instance. Use for passing to code expecting the interface.</summary>
	public global::KnockOff.Documentation.Samples.Skill.ISkillNotifier Object => this;
	...
	/// <summary>Delegates unconfigured member access to the provided source object (global::KnockOff.Documentation.Samples.Skill.ISkillNotifier).</summary>
	/// <param name="source">The source to delegate to, or null to clear.</param>
	public void Source(global::KnockOff.Documentation.Samples.Skill.ISkillNotifier? source)
	{
		Notify._source = source;
	}

	void global::KnockOff.Documentation.Samples.Skill.ISkillNotifier.Notify(int userId, string message)
	{
		Notify.Invoke(Strict, userId, message);
	}
