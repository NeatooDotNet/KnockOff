...
partial class EmailServiceStub : global::KnockOff.Documentation.Samples.AdvancedCallbacks.IEmailService, global::KnockOff.IKnockOffStub
{
	...
	public sealed class SendInterceptor
	{
		...
		private (string? to, string? message)? _unconfiguredLastArgs;
		...
		public (string? to, string? message)? LastCallArgs { get { if ((_onCallTracking?.CallCount ?? 0) > 0) return _onCallTracking!.LastArgs; if (_sequence != null) for (int i = _sequence.Count - 1; i >= 0; i--) if (_sequence[i].Tracking.CallCount > 0) return _sequence[i].Tracking.LastArgs; return _unconfiguredCallCount > 0 ? _unconfiguredLastArgs : default; } }
		...
		public global::KnockOff.IMethodTrackingArgs<(string? to, string? message)> OnCall(SendDelegate callback)
		{
			_sequence = null;
			_sequenceIndex = 0;
			_isVerifiable = false;
		...
		internal bool Invoke(bool strict, string to, string message)
		{
			...
			_unconfiguredLastArgs = ((to, message));
			if (_sequence != null && _sequenceIndex >= _sequence.Count)
			{
			...
			if (_source is { } src) return src.Send(to, message);
		...
		private sealed class MethodTrackingImpl : global::KnockOff.IMethodTrackingArgs<(string? to, string? message)>
		{
			...
			private (string? to, string? message) _lastArgs;

			internal int CallCount { get; private set; }

			/// <summary>Last arguments passed to this callback. Default if never called.</summary>
			public (string? to, string? message) LastArgs => _lastArgs;

			/// <summary>Records a call to this callback.</summary>
			public void RecordCall((string? to, string? message) args) { CallCount++; _lastArgs = args; }
			...
			public global::KnockOff.IMethodTrackingArgs<(string? to, string? message)> Verifiable()
			{
			...
			public global::KnockOff.IMethodTrackingArgs<(string? to, string? message)> Verifiable(global::KnockOff.Times times)
			{
				_interceptor._isVerifiable = true;
	...
	/// <summary>The global::KnockOff.Documentation.Samples.AdvancedCallbacks.IEmailService instance. Use for passing to code expecting the interface.</summary>
	public global::KnockOff.Documentation.Samples.AdvancedCallbacks.IEmailService Object => this;
	...
	/// <summary>Delegates unconfigured member access to the provided source object (global::KnockOff.Documentation.Samples.AdvancedCallbacks.IEmailService).</summary>
	/// <param name="source">The source to delegate to, or null to clear.</param>
	public void Source(global::KnockOff.Documentation.Samples.AdvancedCallbacks.IEmailService? source)
	{
		Send._source = source;
	}
