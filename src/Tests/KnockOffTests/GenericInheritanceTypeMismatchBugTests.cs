...
public class GenericInheritanceTypeMismatchBugTests
{
	...
	public void GenericInheritance_DerivedMethod_CanBeCalled()
	{
		...
		var typedTracking = stub.Execute.Call((ISampleTarget target, CancellationToken? token) => Task.FromResult<ISampleResult>(expectedResult));
	...
	public void GenericInheritance_BaseMethod_CanBeCalled()
	{
		...
		var expectedResult = new SampleResult { Success = true };
		var baseTracking = stub.Execute.Call((ISampleRuleTarget target, CancellationToken? token) => Task.FromResult<ISampleResult>(expectedResult));
	...
	public void GenericInheritance_BothMethods_TrackSeparately()
	{
		...
		var typedTracking = stub.Execute.Call((ISampleTarget target, CancellationToken? token) => Task.FromResult<ISampleResult>(result1));
		var baseTracking = stub.Execute.Call((ISampleRuleTarget target, CancellationToken? token) => Task.FromResult<ISampleResult>(result2));
		...
		typedTracking.Verify(Called.Once);
		baseTracking.Verify(Called.Once);
	}
	...
	public async Task GenericInheritance_OnCallCallback_WorksForTypedMethod()
	{
		...
		var expectedResult = new SampleResult { Success = true };
		stub.Execute.Call((ISampleTarget target, CancellationToken? token) => Task.FromResult<ISampleResult>(expectedResult));
...
/// - ISampleRule.Call(ISampleRuleTarget, ...)
/// - ISampleRule{T}.Call(T, ...)
///
