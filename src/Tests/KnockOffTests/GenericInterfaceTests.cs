...
public class GenericInterfaceTests
{
	...
	public void GenericInterface_MethodsWork()
	{
		...
		var tracking = knockOff.Save.OnCall((user) => { });
	...
	{
		var knockOff = new UserRepositoryKnockOff();
		var tracking = knockOff.GetById.OnCall((id) => null);
		IRepository<User> repo = knockOff;
	...
	{
		var knockOff = new UserRepositoryKnockOff();
		var tracking = knockOff.GetByIdAsync.OnCall((id) => Task.FromResult<User?>(null));
		IRepository<User> repo = knockOff;
