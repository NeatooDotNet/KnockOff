...
public class StandaloneClassStubTests
{
	...
	{
		var stub = new ServiceBaseStub("TestService");
		stub.Execute.Call((cmd) => cmd.Length * 10);
	...
	{
		var stub = new ServiceBaseStub();
		stub.Execute.Call((cmd) => 42).Verifiable();
	...
	{
		var stub = new ServiceBaseStub();
		stub.Execute.Call((cmd) => 1).ThenReturn((cmd) => 2).ThenReturn((cmd) => 3);
	...
	{
		var stub = new ServiceBaseStub();
		stub.Execute.Call((cmd) => 1).ThenReturn((cmd) => 99);
...
public class AbstractStandaloneClassStubTests
{
	...
	{
		var stub = new AbstractServiceStub();
		stub.ExecuteQuery.Call((query) => query.Length);
