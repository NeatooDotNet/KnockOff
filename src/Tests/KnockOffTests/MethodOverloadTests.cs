...
public class MethodOverloadTests
{
    ...
    public void OnCall_DifferentOverloads_CompilerResolvesCorrectly()
    {
        ...
        var tracking1 = stub.Format.OnCall((string input) => input.ToUpper());
        var tracking2 = stub.Format.OnCall((string input, bool uppercase) => uppercase ? input.ToUpper() : input);
        var tracking3 = stub.Format.OnCall((string input, int maxLength) => input.Substring(0, Math.Min(input.Length, maxLength)));
    ...
    public void OnCall_EachOverload_TracksIndependently()
    {
        ...
        var tracking1 = stub.Format.OnCall((string input) => "1");
        var tracking2 = stub.Format.OnCall((string input, bool uppercase) => "2");
