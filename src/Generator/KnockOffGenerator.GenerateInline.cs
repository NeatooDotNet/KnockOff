...
public partial class KnockOffGenerator
{
	...
	/// Uses the new model + renderer pattern for interfaces and delegates,
	/// with legacy fallback for class stubs (until Phase 7 is complete).
	/// </summary>
	private static void GenerateInlineStubs(SourceProductionContext context, InlineStubClassInfo info)
	{
		...
		// If there are class stubs, use legacy generation path for now
		// (until Phase 7: Class Model + Renderer is complete)
		if (info.Classes.Count > 0)
		{
			GenerateInlineStubsLegacy(context, info);
			return;
		}

		// Use new model + renderer pattern for interfaces and delegates
	...
	/// <summary>
	/// Legacy implementation of inline stub generation (StringBuilder-based).
	/// Used when class stubs are present until Phase 7 is complete.
	/// </summary>
	private static void GenerateInlineStubsLegacy(SourceProductionContext context, InlineStubClassInfo info)
	{
