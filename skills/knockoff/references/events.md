...
stub.Started.HasSubscribers; // false

source.Started += (s, e) => { };
stub.Started.HasSubscribers; // true

source.Started -= handler;
stub.Started.HasSubscribers; // false (after removing all handlers)
```
...
stub.Verify(); // Checks all Verifiable() marked members including events
```
...
// Fire events
stub.Started.Raise(source, EventArgs.Empty);
stub.DataReceived.Raise(source, new DataEventArgs("test"));
