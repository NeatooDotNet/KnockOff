...
Comprehensive guide for migrating Moq tests to KnockOff.

## Quick Reference

| Moq | KnockOff |
|-----|----------|
| `new Mock<IService>()` | `new ServiceStub()` or `new Stubs.IService()` |
...
| `It.Is<T>(pred)` | Check predicate in callback body |
...
**Key difference:** No `It.IsAny<T>()` - callback receives actual arguments.
...
**KnockOff (automatic tracking):**
```csharp
// No setup needed!
service.Save(user);
var captured = stub.Save.LastArg;
```
...
**KnockOff (callback):**
```csharp
stub.GetConfig.OnCall((ko) => new Config { Timeout = 30 });
```
...
**Alternative (inline - supports multiple):**
```csharp
...
Not available in Moq - define shared behavior in the stub class:
...
// Override per-test
stub.GetById.OnCall((ko, id) => new User { Id = id, Name = "Override" });
```
...
Assert.Equal(42, stub.Save.LastArg?.Id);
...
## Moq Features Not in KnockOff

| Moq Feature | KnockOff Equivalent / Alternative |
|-------------|-----------------------------------|
| `It.IsAny<T>()` | Implicit - callback receives actual args |
| `It.Is<T>(pred)` | Check predicate in callback |
| `SetupSequence` | Use `Queue<T>` + `Dequeue()` |
| `.As<T>()` | Create separate stubs or use interface inheritance |
| `MockBehavior.Strict` | `.Strict()` extension method |
| `VerifyNoOtherCalls` | Not supported |
| `Protected()` | Not supported |
| `MockRepository` | Not needed (stubs are classes) |
...
See [strict-mode.md](strict-mode.md) for details.

## Summary

**Quick decision guide:**

| Moq Pattern | KnockOff Pattern |
|-------------|------------------|
| Simple mock | Inline stub |
| Shared mock | Standalone stub with user methods |
| Strict mode | `.Strict()` extension |
| Multiple interfaces | Separate stubs or inline |
| Argument capture | Automatic tracking |
| Conditional setup | Switch in OnCall |
